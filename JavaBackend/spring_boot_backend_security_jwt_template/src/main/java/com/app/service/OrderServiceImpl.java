...
public class OrderServiceImpl implements OrderService {
	...
	@Override
	public PlaceOrderDTO placeOrder(String email) {
		// getting the list of book with quantity of specific customer
		List<BookQty> bookQtys =bookQtyDao.findByUserEmail(email); //no unique in the bookQtys;
		//check for cart empty condition
		if(bookQtys.isEmpty())
			return null;
		//new order object transient -> persistent
		Order order = orderDao.save(new Order(null,OStatus.ORDERED,LocalDate.now(),LocalDate.now().plusDays(5),0,userDao.findByEmail(email).orElseThrow(()-> new ResourceNotFoundException("Customer not found"))));
	...
	public List<OrderDTO> getAllOrders(String email) {
		return  orderDao.findByUserEmail(email).stream().map(order -> mapper.map(order, OrderDTO.class)).collect(Collectors.toList());	
	}
